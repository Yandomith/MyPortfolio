...
// Custom cursor removed: native cursor is used instead.

// Microinteraction: add quick 'pressed' class on pointer down/up for tactile feedback
document.addEventListener("DOMContentLoaded", () => {
  const interactiveSelector = "a, button, .card, .glass-card, .nav-link";

  function findInteractive(el) {
    return el.closest ? el.closest(interactiveSelector) : null;
  }
