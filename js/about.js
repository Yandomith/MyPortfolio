function renderExperience() {
  ...
  list.innerHTML = ABOUT_DATA.experience
    .map(
      (item, index) => `
...
function renderEducation() {
  ...
  list.innerHTML = ABOUT_DATA.education
    .map(
      (item) => `
        <div>
            <div class="d-flex justify-content-between align-items-baseline mb-2">
...
function renderSkills() {
  ...
  let html = "";
  const skillGroups = Array.isArray(ABOUT_DATA.skills) ? ABOUT_DATA.skills : Object.entries(ABOUT_DATA.skills).map(([category, items]) => ({ category, items }));
